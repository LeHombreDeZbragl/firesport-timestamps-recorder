...
```bash
# -------- Input videos ---------
# Create a folder treninky, inside another for this date, and inside another named in.
# There you will put all the videos you want to process in .mp4 or .mov format.
# Best practise is to put it in order for better orientation later.
run.bat join -p treninky\datum\in -O input.mp4
# or
make join FOLDER=treninky/datum/in 
# Creates joined input.mp4 in the date folder.
# ------------ GUI -------------
run.bat gui
# or
make gui 
# Load input.mp4
# If you can rely on the mark light
  # Input the the max(LP, PP) to the relative timer
  # Input the order of this attack (Útok 1)
  # Fill in the rest of the splits (the min(LP, PP) can be inputted directly in the relative timer field)
# If you can't rely on the mark light
  # Get the first movement and the start and press E
  # Input the order of this attack (Útok 1)
  # Press R to subtract 150ms from the start split (compensate reaction time)
  # Fill in the rest of the splits (the LP and PP can be inputted directly in the relative timer fields)
# export timestamps.txt to the date folder
# --------- Cut & Output --------
run.bat cut -s treninky\datum\input.mp4 -t treninky\datum\timestamps.txt
# or
make cut SOURCE=treninky/datum/input.mp4 TIMES=treninky/datum/timestamps.txt
# Output: out-parts/ folder with individual attacks + date_final.mp4 both in the date folder
```
